...
Deterministic, per-git-worktree isolation of your app's **dev/test databases** and
**dev-server port** — so you can run many linked worktrees (one per feature, one per agent)
in parallel without them clobbering each other's databases or fighting over port 4000.

No generated manifest, no `.local.exs` files to manage, no manual port bookkeeping. The
values are *derived* from the worktree itself. Built for Phoenix — the installer speaks
`phx.new`'s config shapes — but the core is plain Elixir/Mix with zero framework dependencies.
...
cd ../my-app-feature-x
mix deps.get
mix ecto.create && mix ecto.migrate   # creates my_app_dev_my_app_feature_x
mix phx.server                         # boots on its own port
```

Tear it down by dropping its suffixed databases and `git worktree remove`.
